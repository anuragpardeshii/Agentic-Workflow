...
const CodeEditor = forwardRef(({ json, currentView, isExpanded }, ref) => {
    ...
    refreshFiles: (newFiles) => {
      ...
      if (vmRef) {
        ...
        vmRef.getFsSnapshot().then((snapshot) => {
          ...
          const filesToDestroy = existingFiles.filter(
            (f) =>
  ...
  const handleEditorChange = (newValue) => {
    ...
    try {
      const currentJsonData = JSON.parse(
  ...
  const renderFileTree = () => {
      ...
      return Object.keys(structure).map((name) => {
        ...
        if (structure[name] === null) {
          ...
          return (
            <div
              key={fullPath}
              className={`pl-4 py-1 cursor-pointer hover:bg-purple-800/30 ${
    ...
    return (
      <div className="text-sm  text-white overflow-y-auto h-full bg-purple-900/10 border-r border-purple-500/20">
  ...
  return (
    <div className="h-full">
      <div
        className={`h-full ${currentView === "preview" ? "block" : "hidden"}`}
      >
      ...
      <div
        className={`h-full ${
          currentView === "editor" ? "flex flex-col" : "hidden"
        }`}
      >
        ...
        <div className="flex flex-1 h-[calc(100%-40px)]">
          <div className="w-64 h-full">{renderFileTree()}</div>
