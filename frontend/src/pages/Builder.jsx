...
const Builder = () => {
  ...
  const parseJSON = localStorage.getItem("jsonData")
    ...
    try {
      ...
      if (parsedContent) {
        ...
        if (editorRef.current?.refreshFiles) {
          ...
          Object.entries(parsedContent.jsonData.files).forEach(
            ([path, fileData]) => {
  ...
  const handleCompile = () => {
    ...
    try {
      ...
      vm.getFsSnapshot().then((snapshot) => {
        ...
        const currentFiles = Object.keys(files).map((f) =>
          f.startsWith("/") ? f : `/${f}`
        );
            ...
            <div className="flex items-center bg-purple-900/20 rounded-xl border border-purple-500/20 p-2">
              <Play
                className={`w-4 h-4 mr-2 ${
                  isLoading
                    ? "text-purple-400"
                    : "text-purple-500 hover:text-purple-200 cursor-pointer"
                }`}
              ...
              <input
                ...
                onKeyPress={(e) =>
                  e.key === "Enter" && generatingUpdatingContent()
                }
